...
const Navbar = () => {
  ...
  const [activeOffer, setActiveOffer] = useState("✨ Welcome to SriNails | Premium Press-on Nails 💅 ✨");
    ...
    axios.get('/api/coupons/latest-active')
      .then(res => {
        if (res.data) {
          ...
          if (offerType === 'BOGO') {
            offerText = `✨ BUY 1 GET 1 FREE on orders over ₹${minPurchase}! Use code: ${code} ✨`;
          } else if (offerType === 'Flat') {
            offerText = `✨ ₹${discountValue} OFF on orders over ₹${minPurchase}! Use code: ${code} ✨`;
          } else {
            offerText = `✨ ${discountValue}% OFF on orders over ₹${minPurchase}! Use code: ${code} ✨`;
          }
