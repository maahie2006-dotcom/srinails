...
const Home = () => {
  ...
  const handleSubscribe = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Ensure your backend has this route: /api/newsletter/subscribe
      const res = await axios.post('/api/newsletter/subscribe', { email });
      
      if (res.status === 200 || res.status === 201) {
        toast.success("Welcome to the SriNails family! ✨");
        setIsSubscribed(true); // Shows the success message
        setEmail('');
      }
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to subscribe. Try again later! 💅");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <section className="newsletter-section">
        <div className="container">
          <div className="newsletter-inner">
            {isSubscribed ? (
              // ✅ Success Message
              <div className="newsletter-success" style={{textAlign: 'center', padding: '20px'}}>
                <span style={{fontSize: '3rem'}}>💌</span>
                <h2>You're on the list!</h2>
                <p>Check your inbox for your <strong>10% OFF</strong> welcome code. Stay glamorous! 💅</p>
              </div>
            ) : (
              // ✅ Subscription Form
              <>
